...
public static class TeamData {
    ...
    static TeamAppointment[] CreateTrainingAppointments(DateTime start) {
        ...
        apt.RecurrenceInfo = recurrenceInfo.ToXml();

        int occurrenceCount = RecurrenceHelper.CalcWeeklyOccurrenceIndex(recurrenceInfo, newStart);
        var changedOccurrence = new TeamAppointment {
            AppointmentType = (int)AppointmentType.ChangedOccurrence,
            AllDay = false,
            Start = Start.AddDays(1),
            End = Start.AddDays(1).AddHours(1.5),
            Subject = "Sport Training",
            Status = 1,
            Label = 3,
            CalendarId = 0,
            RecurrenceInfo = RecurrenceHelper.ToPatternReference(recurrenceInfo, occurrenceCount),
            ReminderInfo = ReminderHelper.ToReminderXml(TimeSpan.FromMinutes(15))
        };
        return [apt, changedOccurrence];
    }
